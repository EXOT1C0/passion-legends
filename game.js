(() => {
  ...
  function buildRoster() {
    ...
    Object.values(CHARACTERS).forEach(character => {
      ...
      card.innerHTML = `
        ...
        <span class="fighter-glyph">${character.glyph}</span>
  ...
  function renderActors() {
    for (const actor of game.actors) {
      ...
      if (actor.invulnerable > 0 && Math.floor(actor.invulnerable * 12) % 2 === 0) ctx.globalAlpha = .45;
      ctx.shadowColor = actor.data.color; ctx.shadowBlur = actor.flash > 0 ? 30 : 14;
      ctx.fillStyle = actor.flash > 0 ? '#fff' : '#0c1420';
      ctx.beginPath(); ctx.arc(0, 0, actor.radius, 0, Math.PI * 2); ctx.fill();
      ctx.lineWidth = actor.human !== null ? 4 : 2.5; ctx.strokeStyle = actor.data.color; ctx.stroke();
      ctx.shadowBlur = 0;
      ctx.fillStyle = actor.data.color; ctx.font = `950 ${actor.radius}px system-ui`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(actor.data.glyph, 0, 1);
      ctx.strokeStyle = actor.data.color; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(actor.facingX * (actor.radius - 4), actor.facingY * (actor.radius - 4)); ctx.lineTo(actor.facingX * (actor.radius + 10), actor.facingY * (actor.radius + 10)); ctx.stroke();
      if (actor.human !== null) {
        ctx.fillStyle = '#fff'; ctx.font = '900 8px system-ui'; ctx.fillText(`P${actor.human + 1}`, 0, -actor.radius - 16);
      }
      ctx.restore();

      const barW = 48;
      ctx.fillStyle = 'rgba(0,0,0,.65)'; ctx.fillRect(actor.x - barW / 2, actor.y + actor.radius + 10, barW, 5);
      ctx.fillStyle = actor.team === 'alpha' ? '#55e7ff' : '#ff5575'; ctx.fillRect(actor.x - barW / 2, actor.y + actor.radius + 10, barW * (actor.hp / actor.maxHp), 5);
      if (actor.shield > 0) { ctx.fillStyle = '#d3f7ff'; ctx.fillRect(actor.x - barW / 2, actor.y + actor.radius + 17, barW * Math.min(1, actor.shield / 100), 2); }
    }
  }
  ...
  function renderDecoys() {
    for (const decoy of game.decoys) {
      ctx.save(); ctx.translate(decoy.x, decoy.y); ctx.globalAlpha = decoy.type === 'phantom' ? .58 : .72;
      ctx.shadowColor = decoy.color; ctx.shadowBlur = 14; ctx.fillStyle = hexToRgba(decoy.color, .18);
      ctx.beginPath(); ctx.arc(0, 0, decoy.radius, 0, Math.PI * 2); ctx.fill();
      ctx.setLineDash(decoy.type === 'phantom' ? [4, 3] : [7, 3]); ctx.strokeStyle = decoy.color; ctx.lineWidth = 2; ctx.stroke(); ctx.setLineDash([]);
      ctx.shadowBlur = 0; ctx.fillStyle = decoy.color; ctx.font = '950 17px system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(decoy.glyph, 0, 1);
      ctx.font = '800 7px system-ui'; ctx.fillText(decoy.type === 'phantom' ? 'REAL?' : 'DECOY', 0, -27);
      ctx.restore();
    }
  }
